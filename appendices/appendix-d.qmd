...
atlas_to_ozfs <- function(
    nza_file_path, #Path to one NZA file (must be xlsx or csv)
  ...
  ozfs_format <- list(type = "FeatureCollection",
                      ...
                      definitions = list(
                        ...
                        res_type = list(
                          list(condition = "total_units == 1",
                               expression = "'1_unit'"),
                          list(condition = "total_units == 2",
                               expression = "'2_unit'"),
                          list(condition = list("total_units > 2",                                                               "n_outside_entry == total_units",
                                                "n_ground_entry == total_units",                                                 "sep_platting == TRUE"),
                               expression = "'townhome'"),
...
organize_feature <- function(atlas_row_df, 
  ...
  for (i in 1:length(use_type_indicators)){
    ...
    if (!is.na(filtered_columns$use_permitted) & 
        filtered_columns$use_permitted == "Allowed/Conditional"){
  ...
  if (nrow(constraints_df) > 0){
    features_list$properties$constraints <- 
      make_constraints(constraints_df, separated_uses)
  }
  ...
  for (i in 1:nrow(constraints_df)){
    ...
    for (minmax_loop in 1:2){
      ...
      for (j in 1:length(separated_uses)){
        ...
        # we make either of those scenarios NULL so that it gets the 
        # proper use groupings
      ...
      key_vec <- sapply(filtered_separated_uses, 
                        function(x) paste(serialize(x, NULL), collapse = "-"))
      ...
      for (use_group in grouped_uses){
        ...
        if (grouped_uses_count > 1 & !is.null(organized_rules)){
          ...
          for (rule_num in 1:length(organized_rules)){
            organized_rules[[rule_num]]$condition <- 
              append(organized_rules[[rule_num]]$condition, list(use_condition))
          }
        }
      ...
      if (length(rule_list) > 0){
        constraints_list[[constraint_name]][[minmax_vals[[minmax_loop]]]] <-
          rule_list
      } 
    }
...
organize_rules <- function(df_with_rules, constraint_name){
  ...
  while (ncol(df) > 0){
    counter <- counter + 1
    df <- df_with_rules[1,
                        grep(paste0("rule", counter), 
                             names(df_with_rules))]
  }
  
  rules_count <- counter - 1
  
  df_just_rules <- df_with_rules[1,
                                 grep(paste0(constraint_name, "_rule"), 
                                      names(df_with_rules))]
  ...
  for (i in 1:rules_count){
    ...
    rule_df <- df_with_rules[,
                            grep(paste0(constraint_name,
                                          "_rule", i), 
                                 names(df_with_rules))]
    ...
    if (sum(grep("logical_operator", names(rule_df))) > 0 ){
      ...
      logical_operator <- rule_df[[1, 
                                   grep("logical_operator",
                                        names(rule_df))]]
    }
    ...
    if (sum(grep("condition", names(rule_df))) > 0 ){
      ...
      if (is.null(logical_operator)){
        ...
        for (j in 1:ncol(condition_df)){
          ...
          condition_value <- paste(condition_list, 
                                   collapse = paste0(" ", 
                                                     tolower(logical_operator),
                                                     " "))
        }
        
      }
      rule_list$condition <- append(rule_list$condition,
    ...
    if (sum(grep("criterion", names(rule_df))) > 0 ){
      ...
      if (criterion == "dependent"){
        if (sum(grep("more_restrictive", 
                     names(rule_df))) > 0 ){
          rule_list$condition <- append(rule_list$condition, 
                                        rule_df[[1, 
                                                 grep("more_restrictive",
                                                      names(rule_df))[1]]])
        } else{ # there is no explanation for some reason
          rule_list$condition <- append(rule_list$condition, 
                                        "Special condition that wasn't stated")
        }
      } else{
        rule_list$min_max <- rule_df[[1, 
                                      grep("criterion",
                                           names(rule_df))[1]]]
      }
    }
    ...
    if (sum(grep("expression", names(rule_df))) > 0 ){
      ...
      df_expression <- rule_df[, 
                               grep(paste0("expression"), 
                                    names(rule_df))]
...
add_geometry_to_ozfs <- function(boundary_file_path, ozfs_list){
  ...
  for (i in 1:length(ozfs_list$features)){
    ...
    for (j in 1:length(boundaries$features)){
      boundary_dist_name <- 
        boundaries$features[[j]]$properties$`Abbreviated District Name`
...
add_extra_overlays <- function(extra_overlay_geom_file, ozfs_list){
  ...
  for (k in 1:length(extra_overlays$features)){
    if (extra_overlays$features[[k]]$properties$muni_name == 
        ozfs_list$muni_name){
  ...
  for (i in city_idx){
    ...
    if (class(feature) == "list"){ # if the feature exists, we add geometry
      ...
      overlay_feature_i$properties$planned_dev <- 
        ifelse(abbr %in% c("PD","PRD"),TRUE,FALSE)
