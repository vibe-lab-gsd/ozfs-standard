...
2. Jurisdiction-specific definitions. Each feed contains an explicit glossary for locally defined terms—e.g., townhouse, multifamily, cluster development—so that uniform semantics are not imposed.
3. Python-syntax mathematical expressions. Controls may be stored as formulas (`MaxHeight` = 0.5 * `LotDepth`), allowing rules that reference parcel or building attributes without manual precalculation.
...
6. Zoning-parcel–building rectification protocol. A preprocessing protocol in Python and R aligns lot edges and centroids with zoning districts, enabling automated feasibility checks.
...
The `sep_platting` key is used to indicate whether each until in the building would be on a separately platted parcel and the `unit_separation` key describes how units are separated (by a "party_wall", a "fire_wall", or an "open_area"). These keys may be used to determine whether the building meet's a municipality's definition of a townhome. There is also an optional `parking` key to indicate the number of parking spaces contained within the building's structure (i.e. in a garage).
...
The current schema records all base districts and overlays together, identifying districts as overlays but does not yet capturing their regulations nor priority rules to help reconcile overlays with the base district. Future work should:
- allow multiple simultaneous overlays (e.g. “Downtown Core” + “Inclusionary Housing”);
- provide a deterministic precedence model so applications can resolve conflicts among base and overlay provisions;
- support geometry-based overlays defined mathematically, for example: “within a quarter mile of a transit station” or “2000 feet outside of an airport runway”; and
- store or reference official and unofficial environmental layers (wetlands, riverfronts, FEMA flood plains, site grading) with rigor sufficient for zoning use.
...
Maximum height is only one of many constraints operating in the vertical dimension or Z-plane. Spatially-defined planes limiting bulk or defining view corridors are common. Other constraints only take effect at or above specific elevations: step-backs, tower separation, and penthouse max floorplates. A future OZFS enhancement should:
- allow parametric planes and angular envelopes, not just fixed limits;
- expand the roof-typology catalogue beyond the initial set; and
- be able to reference these planes in conditional expressions
...
Single GeoJSON files per jurisdiction maximizes human readability and permits version tracking with Git, but we foresee drawbacks with the current approach, including in terms of processing speed and indexing, topological checks, and file size. Some potential areas for optimization are:
- separating districts and overlays into multiple geospatial layers (potentially using GeoPackage)
- separate geospatial and zoning files (to enable readability by standard GIS software)

Future guidance should specify how feeds can be stored and partitioned without breaking referential integrity.

## Split districts

Where a parcel straddles two districts, jurisdictions differ on whether the parcel must be subdivided, may distribute entitlements, or must apply the most restrictive rule.  A subsequent version may encode:
- a sub-parceling routine to generate regulatory sublots;
- jurisdiction-specific precedence flags (e.g. “restrictive-prevails” vs “pro-rated FAR”); and
- per-municipality reconciliation logic.
