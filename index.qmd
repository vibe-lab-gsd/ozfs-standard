...
The OZFS data standard uses a geojson data format to describe zoning regulations, which offers the flexibility to define only those variables that are defined for a particular zoning district and to add additional key-value pairs for new variables as needed. We incorporate the use of expressions (in the Python syntax) and nested arrays to define constraints that are dependent on land use or on characteristics of buildings and parcels, as well as to define municipality-level definitions of relating to building characteristics. To ensure that buildings and parcels are described in a way that is interpretable by these expressions, the OZFS data standard also includes standards for encoding building and parcel characteristics.
