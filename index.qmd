...
Zoneomics offers data through a paid API subscription. API outputs are in a json or geojson format, which offers the potential to overcome a weakness of a tabular data format: variables do not necessarily need to be stored for any districts they are not defined for. As noted by Kahn [-@kahn_nza_2024] for NZA data published by the Mercatus Center, some variable values are necessarily encoded by non-standardized text strings that describe constraints that may depend on land use or on characteristics of the building of the parcel, and these text strings cannot be consistently interpretted by software.
...
(Presumably) inside individual firms, bespoke JSON or GraphQL schemas now power automated tools and linked formulas. None, however, has been published as a public, versioned specification that cities, researchers, and software vendors can adopt without reinvention.
